...
{/if}
{if $order->gift}
	<p><img src="{$img_dir}icon/gift.png" alt="" class="icon" />&nbsp;{l s='You have requested gift wrapping for this order.'}</p>
...
<div id="order-detail-content" class="table_block">
	<table id="order-detail-content-table" class="std shop_table footable">
		<thead>
			<tr>
            	<th  data-class="expand" class="item">{l s='Product'}</th>
				{if $return_allowed}<th data-hide="phone"  class="first_item "><input type="checkbox" /></th>{/if}
				<th data-hide="phone" class="{if $return_allowed}item{else}first_item{/if}">{l s='Reference'}</th>
				<th data-hide="phone" class="item">{l s='Quantity'}</th>
				{if $order->hasProductReturned()}
					<th class="item">{l s='Returned'}</th>
				{/if}
				<th data-hide="phone" class="item">{l s='Unit price'}</th>
				<th data-hide="phone" class="last_item">{l s='Total price'}</th>
			</tr>
		</thead>
		<tfoot>
...
{if $order->getShipping()|count > 0}
	<table id="shipping-table" class="std shop_table footable">
		<thead>
			<tr>
				<th data-class="expand"  class="first_item">{l s='Date'}</th>
				<th data-hide="phone" class="item">{l s='Carrier'}</th>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)}" method="post" class="std titled_box" id="sendOrderMessage">
		...
		<p class="submit">
			...
			<input type="submit" class="btn btn-inverse" name="submitMessage" value="{l s='Send'}"/>
		</p>
	</form>
{else}
<p><img src="{$img_dir}icon/infos.png" alt="" class="icon" />&nbsp;{l s='You cannot return merchandise with a guest account'}</p>
{/if}
