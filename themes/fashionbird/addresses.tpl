...
<div class="addresses titled_box">
	...
	{foreach from=$multipleAddresses item=address name=myLoop}
		<ul class="address {if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}">
			...
			{/foreach}
			<li class="address_update"><a class="btn btn-success" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")}" title="{l s='Update'}">{l s='Update'}</a></li>
			<li class="address_delete"><a class="btn btn-danger" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")}" onclick="return confirm('{l s='Are you sure? js=1'}');" title="{l s='Delete'}">{l s='Delete'}</a></li>
		</ul>
	{/foreach}
	</div>
	<p class="clear" />
...
{else}
	<p class="warning"><i class="icon-info-sign"></i>{l s='No addresses are available.'}&nbsp;<a href="{$link->getPageLink('address', true)}">{l s='Add a new address'}</a></p>
{/if}
