...
{if !$opc}<h1><span>{l s='Addresses'}</span></h1>{else}<h2><span>1</span> {l s='Addresses'}</h2>{/if}
	...
	{if !$multi_shipping && {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
		<div class="button_multishipping_mode titled_box" id="multishipping_mode_box">
			...
			<div class="description">
				<a class="btn btn-large btn-block" href="{$link->getPageLink('order', true, NULL, 'step=1&multi-shipping=1')}"/>
...
{/if}
	<div class="addresses clearfix">
		...
		<p style=" margin-bottom:15px;" class="checkbox addressesAreEquals" {if $cart->isVirtualCart()}style="display:none;"{/if}>
		...
		<p id="address_invoice_form" style=" padding-top:0" class="select" {if $cart->id_address_invoice == $cart->id_address_delivery}style="display: none;"{/if}>
		...
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="btn btn-success">{l s='Add a new address'}</a>
		</p>
		{if !$opc}
