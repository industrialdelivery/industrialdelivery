...
if ($(document.body).width() < 751){
				if ($picture.size())
						$picture.css({'position': 'absolute', 'width':40, 'height':61});
	}
if ($(document.body).width() > 751){
				if ($picture.size())
					$picture.css({'position': 'absolute', 'width':100, 'height':153 , 'top': pictureOffsetOriginal.top, 'left': pictureOffsetOriginal.left});
	}
	// end custom
				...
				// add custom
					.animate({ 'width': $element.attr('width')*0.66, 'height': $element.attr('height')*0.66, 'opacity':1, 'top': cartBlockOffset.top + 50, 'left': cartBlockOffset.left + 1 }, 1500, 'linear')
...
$('body').append('<div class="alert_cart" style=" display:none;"><a class="mas_cart"  href="index.php?controller=order">Your product has been successfully added to your cart!</a></div>');
...
$('.alert_cart').append('<span title="Close" class="alert_cart_close"></span>');
               ...
               $('.alert_cart').slideUp(400);
                 });

  var msg = $('.alert_cart').slideDown(400);
  timer = setTimeout(function() { msg.slideUp(400); }, 7000);
					...
					if (removedProductId != null)
					{
						...
						$('#'+removedProductId).addClass('strike').fadeTo('slow', 0, function(){
							$(this).slideUp('slow', function(){
								...
								{
									$('#cart_block_no_products:hidden').removeClass('no_products').addClass('cart_no_products').slideDown(450);
		...
		$(jsonData.products).each(function(){
			...
			if (this.id != undefined)
			{
				...
				if ($('#cart_block dl.products').length == 0)
				{
					...
					$('#cart_block_no_products').removeClass('cart_no_products').addClass('no_products').hide();
				}
