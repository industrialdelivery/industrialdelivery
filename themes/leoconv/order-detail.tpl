...
<form action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
	<div>
		...
		<p>
			<input type="submit" value="{l s='Reorder'}" name="submitReorder" class="button exclusive btn" />
...
{if !$is_guest}
	...
	<div id="returnOrderMessage">
		...
		<p class="submit">
			<input type="submit" value="{l s='Make an RMA slip'}" name="submitReturnMerchandise" class="button_large btn" />
	...
	{/if}
	<form action="{$link->getPageLink('order-detail', true)}" method="post" class="std form-horizontal" id="sendOrderMessage">
