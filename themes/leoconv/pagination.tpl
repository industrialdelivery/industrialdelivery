...
{if isset($p) AND $p}
	...
	{if $start!=$stop}
		<p>{l s='Page:' mod='pagination'}</p>
		<ul class="pagination">
		{if $p != 1}
			{assign var='p_previous' value=$p-1}
			<li id="pagination_previous"><a class="btn" {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}">&laquo;&nbsp;</a></li>
		{else}
			<li id="pagination_previous" class="disabled"><span class="btn" >&laquo;&nbsp;</span></li>
		{/if}
		{if $start==3}
			<li><a class="btn"  {$no_follow_text}  href="{$link->goPage($requestPage, 1)}">1</a></li>
			<li><a  class="btn" {$no_follow_text}  href="{$link->goPage($requestPage, 2)}">2</a></li>
		{/if}
		{if $start==2}
			<li><a {$no_follow_text}  class="btn"  href="{$link->goPage($requestPage, 1)}">1</a></li>
		{/if}
		{if $start>3}
			<li><a {$no_follow_text}  class="btn"  href="{$link->goPage($requestPage, 1)}">1</a></li>
		...
		{section name=pagination start=$start loop=$stop+1 step=1}
			{if $p == $smarty.section.pagination.index}
				<li class="current"><span class="btn" >{$p|escape:'htmlall':'UTF-8'}</span></li>
			{else}
				<li><a {$no_follow_text}  class="btn" href="{$link->goPage($requestPage, $smarty.section.pagination.index)}">{$smarty.section.pagination.index|escape:'htmlall':'UTF-8'}</a></li>
			{/if}
		{/section}
		{if $pages_nb>$stop+2}
			<li class="truncate">...</li>
			<li><a  class="btn" href="{$link->goPage($requestPage, $pages_nb)}">{$pages_nb|intval}</a></li>
		{/if}
		{if $pages_nb==$stop+1}
			<li><a  class="btn" href="{$link->goPage($requestPage, $pages_nb)}">{$pages_nb|intval}</a></li>
		{/if}
		{if $pages_nb==$stop+2}
			<li><a  class="btn" href="{$link->goPage($requestPage, $pages_nb-1)}">{$pages_nb-1|intval}</a></li>
			<li><a  class="btn" href="{$link->goPage($requestPage, $pages_nb)}">{$pages_nb|intval}</a></li>
		{/if}
		{if $pages_nb > 1 AND $p != $pages_nb}
			{assign var='p_next' value=$p+1}
			<li id="pagination_next"><a  class="btn" {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}">&nbsp;&raquo;</a></li>
		{else}
