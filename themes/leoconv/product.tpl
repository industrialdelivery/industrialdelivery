		...
		<div id="pb-right-column" class="span6">
			<div class="row-fuild"> 
				...
				<div id="image-block">
					{if $have_image}
						<span id="view_full_size">
							<a>
								<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')}"{if $jqZoomEnabled} class="jqzoom"{/if} title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" id="bigpic"  />
							</a>
				...
				<div id="views_block" class="{if isset($images) && count($images) < 2}hidden{/if}">
					{if isset($images) && count($images) > 3}<span class="view_scroll_spacer "><a id="view_scroll_left" class="icon-angle-up" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}">&nbsp;</a></span>{/if}
					<div id="thumbs_list">
						...
						{if isset($images) && count($images) > 3}<a id="view_scroll_right" class="icon-angle-down" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}">&nbsp;</a>{/if}
					{/if}
				</div> 
			</div>
...
<!-- quantity discount -->
<ul class="idTabs clearfix block_box_center">
	<li><a href="#discount" style="cursor: pointer" class="selected">{l s='Sliding scale pricing'}</a></li>
</ul>
<div id="quantityDiscount" class="block_box_center">
	<table class="std">
        <thead>
            <tr>
                <th>{l s='Product'}</th>
...
<div id="more_info_block" class="clear block_box_center">
				...
				<div class="block_content">
					...
					{foreach from=$accessories item=accessory name=accessories_list}
						...
						{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<div class="p-item span4 product_block ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
								<div class="product-container clearfix">
									
									<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{$accessory.legend|escape:'htmlall':'UTF-8'}" class="product_image">
										<img src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'home_default')}" alt="{$accessory.legend|escape:'htmlall':'UTF-8'}"   />
									</a>
								
								
									<h5 class="s_title_block">
										<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}">{$accessory.name|escape|truncate:25:'...':true}</a>
									</h5>

									<div class="product_desc">
										<div class="block_description">
											{$accessory.description_short|strip_tags|truncate:60:'...'}
										</div>
						...
						{if ($accessory@iteration%3==0||$smarty.foreach.accessories_list.last)}
							</div>
						{/if}
	...
	{if isset($product) && $product->customizable}
		<div id="idTab10" class="bullet customization_block">
			...
			<p class="clear required"><sup>* {l s='Required field'}</sup></p>
		</div>
	{/if}
...
{if isset($packItems) && $packItems|@count > 0}
	<div id="blockpack" class="block_box_center">
