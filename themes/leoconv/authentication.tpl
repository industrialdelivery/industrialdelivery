...
{if !isset($email_create)}
	...
	<div class="row-fuild">
		<form action="{$link->getPageLink('authentication', true)}" method="post" id="create-account_form" class="std span6 ">
			<fieldset>
				...
				<div class="form_content clearfix">
					...
					<p class="submit">
						...
						<input type="submit" id="SubmitCreate" name="SubmitCreate" class="button_large btn" value="{l s='Create an account'}" />
		...
		<form action="{$link->getPageLink('authentication', true)}" method="post" id="login_form" class="std span6">
			<fieldset>
				...
				<div class="form_content clearfix">
					...
					<p class="submit">
						{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}
						<input type="submit" id="SubmitLogin" name="SubmitLogin" class="button btn" value="{l s='Log in'}" />
					</p>
	...
	<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")}" method="post" id="new_account_form" class="std clearfix form-horizontal">
		<fieldset>
				...
				{if isset($newsletter) && $newsletter}
					<div class="checkbox control-group">
						...
						<div class="controls">
							...
							<label class="help-inline" for="newsletter">{l s='Sign up for our newsletter!'}</label>
						</div>
					</div>
					<div class="checkbox control-group">
						<div class="control-label"></div>
						<div class="controls">
							...
							<label class="help-inline" for="optin">{l s='Receive special offers from our partners!'}</label>
						</div>
					</div>
				{/if}
				...
				{foreach from=$dlv_all_fields item=field_name}
					{if $field_name eq "company"}
						...
						<div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}text control-group">
							<label for="phone" class="control-label" >{l s='Phone'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>*</sup>{/if}</label>
							<div class="controls">
		...
		<p class="cart_navigation required submit">
			<span><sup>* {l s='Required field'}</sup></span>
			<input type="hidden" name="display_guest_checkout" value="1" />
			<input type="submit" class="exclusive btn" name="submitGuestAccount" id="submitGuestAccount" value="{l s='Continue'}" />
		</p>
	</form>
	{/if} 
{else}
...
<div class="content-inner block_box_center"> -->
<h1>{if !isset($email_create)}{l s='Log in'}{else}{l s='Create an account'}{/if}</h1>
<form action="{$link->getPageLink('authentication', true)}" method="post" id="account-creation_form" class="std form-horizontal">
	...
	<fieldset class="account_creation">
		...
		<div class="required password control-group">
			...
			<div class="controls">
				...
				<span class="form_info help-inline">{l s='(Five characters minimum)'}</span>
			</div>
		</div>
		<div class="select control-group">
		...
		<div class="checkbox control-group" >
			...
			<div class="controls">
				...
				<label for="newsletter" class="help-inline">{l s='Sign up for our newsletter!'}</label>
			</div>
		</div>
		<div class="checkbox control-group" >
			<div class="control-label">
			...
			<div class="controls">
				...
				<label for="optin" class="help-inline">{l s='Receive special offers from our partners!'}</label>
			</div>
		</div>
		{/if}
	</fieldset>
	...
	<fieldset class="account_creation">
		...
		<div class="text control-group">
			<label for="" class="control-label" >{l s='Company'}</label>
			<div class="controls">
				<input type="text" class="text  " id="company" name="company" value="{if isset($smarty.post.company)}{$smarty.post.company}{/if}" />
			</div>
		</div>
		<div class="text control-group">
			<label for="siret" class="control-label" >{l s='SIRET'}</label>
			<div class="controls">
				<input type="text" class="text  " id="siret" name="siret" value="{if isset($smarty.post.siret)}{$smarty.post.siret}{/if}" />
			</div>
		</div>
		<div class="text control-group">
			<label for="ape" class="control-label" >{l s='APE'}</label>
			<div class="controls">
				<input type="text" class="text  " id="ape" name="ape" value="{if isset($smarty.post.ape)}{$smarty.post.ape}{/if}" />
			</div>
		</div>
		<div class="text control-group">
			<label for="website"  class="control-label" >{l s='Website'}</label>
			<div class="controls">
	...
	<fieldset class="account_creation">
		...
		{foreach from=$dlv_all_fields item=field_name}
			...
			{elseif $field_name eq "address1"}
				<div class="required text control-group">
					...
					<div class="controls">
						<input type="text" class="text  " name="address1" id="address1" value="{if isset($smarty.post.address1)}{$smarty.post.address1}{/if}" />
						<span class="inline-infos help-inline">{l s='Street address, P.O. Box, Company name, etc.'}</span>
					</div>
				</div>
			{elseif $field_name eq "address2"}
				<div class="text control-group">
					...
					<div class="controls">
						...
						<span class="inline-infos help-inline">{l s='Apartment, suite, unit, building, floor, etc...'}</span>
					</div>
				</div>
			{elseif $field_name eq "postcode"}
				<div class="required postcode text control-group">
		...
		{if isset($one_phone_at_least) && $one_phone_at_least}
			<p class="inline-infos">{l s='You must register at least one phone number.'} <sup>*</sup></p>
		{/if}
		<div class="text control-group">
		...
		<div class="required text control-group" id="address_alias">
			<label for="alias" class="control-label" >{l s='Assign an address alias for future reference.'} <sup>*</sup></label>
			<div class="controls">
	...
	<div class="cart_navigation required submit">
		...
		{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}
		<input type="submit" name="submitAccount" id="submitAccount" value="{l s='Register'}" class="exclusive btn" />
		<span><sup>* {l s='Required field'}</sup></span>
	</div>
</form>
<!-- </div> -->
