...
<div class="addresses" id="featured-products_block_center">
    ...
    <div class="row-fluid form-horizontal form-horizontal-mini">
    {/if}
		<div class="span4 address {if $smarty.foreach.i.last}last_item{elseif $smarty.foreach.i.first}first_item{/if} {if $smarty.foreach.i.index % 2}alternate_item{else}{/if}" id="block_{$product.id_product}_{$product.id_product_attribute}">
			<div class="ajax_block_product wl_item">
				<div class="address_title product_image ">
				...
				<div class="address_name product_infos ">
					<a href="{$link->getProductLink($product.id_product,
						$product.link_rewrite, $product.category_rewrite)}" title="{l s='View' mod='blockwishlist'}">
					...
					{/if}
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="{$product.id_product}_{$product.id_product_attribute}" size="3" value="{$product.quantity|intval}" readonly/>
							</div>
						</div>
						<div class="control-group">
							<label class="control-label">
				...
				<div class="address_address1 clear">
					<div class="btn_action">
						...
						{/if}
						<a href="javascript:;" class="exclusive btn" onclick="WishlistBuyProduct('{$token|escape:'htmlall':'UTF-8'}', '{$product.id_product}', '{$product.id_product_attribute}', '{$product.id_product}_{$product.id_product_attribute}', this, {$ajax});" title="{l s='Add to cart' mod='homefeatured'}">{l s='Add to cart' mod='blockwishlist'}</a>
						{else}
						<span class="exclusive btn">{l s='Add to cart' mod='blockwishlist'}</span>
						{/if}
					</div>
				</div>
			</div>
