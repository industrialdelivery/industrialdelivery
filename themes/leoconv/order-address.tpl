...
{/if}
	<div class="addresses clearfix">
		...
		{if $addresses|@count > 1}
			...
			{else}
				<a style="margin-left: 221px;" href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="button_large btn">{l s='Add a new address'}</a>
			{/if}
		</p>
		<div class="clearfix row-fuild">
		<div class="span6">
		...
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="button_large btn">{l s='Add a new address'}</a>
		</p>
		{if !$opc}
