								...
								<div class="button-container">
									{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
										{if ($product->quantity > 0 OR $product->allow_oosp)}
											<a
												class="button ajax_add_to_cart_button btn btn-outline btn-sm"
												data-id-product="{$product->id}"
										...
										{else}
											<span class="ajax_add_to_cart_button button btn btn-outline disabled btn-sm">
												<span>{l s='Out of stock'}</span>
											</span>
										{/if}
									{/if}
									<a 
									class="button lnk_view btn btn-outline btn-sm" 
									href="{$product->getLink()|escape:'html':'UTF-8'}" 
									title="{l s='View'}">
										<span>{l s='View'}</span>
									</a>
