<?php
require (dirname(__FILE__).'/leotopmenulinks.php');

class BlockLeotopmenu extends Module
{
	private $_menu = '';
	private $_html = '';
	private $user_groups;
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Main Top horizontal menu');
		$this->description = $this->l('Add a new menu on top of your shop based on LeoTheme Theme');
	}

	public function install()
	{
		if (!parent::install() ||
			!$this->registerHook('topNavigation') ||
            !$this->registerHook('header') ||
			!Configuration::updateGlobalValue('MOD_BLOCKMULTITOPMENU_ITEMS', 'CAT1,CMS1,CMS2,PRD1') ||
			!Configuration::updateGlobalValue('MOD_BLOCKMULTITOPMENU_SEARCH', '1') ||
	...
	public function installDb()
	{
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'leolinksmenutop` (
			...
			Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'leolinksmenutop_lang` (
	...
	{
		if (!parent::uninstall() ||
			!Configuration::deleteByName('MOD_BLOCKMULTITOPMENU_ITEMS') ||
			!$this->uninstallDB())
			return false;
		return true;
	}
	...
	{
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'leolinksmenutop`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'leolinksmenutop_lang`');
		return true;
	}
	...
	public function getContent()
	{
		...
		{
			if (Configuration::updateValue('MOD_BLOCKMULTITOPMENU_ITEMS', Tools::getValue('items')))
		...
		else if (Tools::isSubmit('submitBlocktopmenuLinks'))
		{
			...
			{
                LeoMenuTopLinks::add(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
		...
		else if (Tools::isSubmit('submitBlocktopmenuRemove'))
		{
			...
			LeoMenuTopLinks::remove($id_linksmenutop, (int)Shop::getContextShopID());
			Configuration::updateValue('MOD_BLOCKMULTITOPMENU_ITEMS', str_replace(array('LNK'.$id_linksmenutop.',', 'LNK'.$id_linksmenutop), '', Configuration::get('MOD_BLOCKMULTITOPMENU_ITEMS')));
		...
		else if (Tools::isSubmit('submitBlocktopmenuEdit'))
		{
			...
			if (!Tools::isSubmit('link'))
			{
				...
				$tmp = LeoMenuTopLinks::getLinkLang($id_linksmenutop, $id_shop);
			...
			{
				LeoMenuTopLinks::update(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)$id_shop, (int)$id_linksmenutop, (int)$id_linksmenutop);
		...
		<fieldset>
			...
			<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post" id="form">
				...
				<div class="margin-form">
					<input type="text" name="items" id="itemsInput" value="'.Tools::safeOutput(Configuration::get('MOD_BLOCKMULTITOPMENU_ITEMS')).'" size="70" />
				</div>
				</div>
		...
		$links = LeoMenuTopLinks::gets($id_lang, null, (int)Shop::getContextShopID());
		foreach ($links as $link)
		{
			if ($link['label'] == '')
			{
				$link = LeoMenuTopLinks::get($link['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
...
<div class="margin-form">';
		...
		$links = LeoMenuTopLinks::gets((int)$id_lang, null, (int)Shop::getContextShopID());
	...
	{
		return explode(',', Configuration::get('MOD_BLOCKMULTITOPMENU_ITEMS'));
	}
	...
	private function makeMenuOption()
	{
		...
		foreach ($menu_item as $item)
		{
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					$link = LeoMenuTopLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						if (!isset($link[0]['label']) || ($link[0]['label'] == ''))
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = LeoMenuTopLinks::get($link[0]['id_linksmenutop'], (int)$default_language, (int)Shop::getContextShopID());
						}
	...
	private function makeMenu()
	{
		...
		foreach ($menu_items as $item)
		{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'PRD':
					$selected = ($this->page_name == 'product' && (Tools::getValue('id_product') == $id)) ? ' class="active"' : '';
					$product = new Product((int)$id, true, (int)$id_lang);
				...
				case 'CMS_CAT':
					...
					if (count($category))
					{
						$this->_menu .= '<li class="dropdown"><a href="'.$category->getLink().'" class="dropdown-toggle" data-toggle="dropdown">'.$category->name.'<b class="caret"></b></a>';
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = LeoMenuTopLinks::get($link[0]['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
						}
    ...
    public function hookHeader($param){
        $this->context->controller->addCSS($this->_path.'css/blockleotopmenu.css', 'all');
    }
	
	public function hookDisplayHeaderRight( $params ){
		return $this->hookTopNavigation( $params );
	}
	...
	public function hookTopNavigation($param)
	{	
		$this->user_groups =  ($this->context->customer->isLogged() ? $this->context->customer->getGroups() : array(Configuration::get('PS_UNIDENTIFIED_GROUP')));
		$this->page_name = Dispatcher::getInstance()->getController();
		$smarty_cache_id = 'blockleotopmenu-'.$this->page_name.'-'.(int)$this->context->shop->id.'-'.implode(', ',$this->user_groups).'-'.(int)$this->context->language->id.'-'.(int)Tools::getValue('id_category').'-'.(int)Tools::getValue('id_manufacturer').'-'.(int)Tools::getValue('id_supplier').'-'.(int)Tools::getValue('id_cms').'-'.(int)Tools::getValue('id_product');
		$this->context->smarty->cache_lifetime = 31536000;
		Tools::enableCache();
		if (!$this->isCached('blockleotopmenu.tpl', $smarty_cache_id))
		{
			$this->makeMenu();
		...
		$html = $this->display(__FILE__, 'blockleotopmenu.tpl', $smarty_cache_id);
	...
	{
		$this->_clearCache('blockleotopmenu.tpl');
	}
}
