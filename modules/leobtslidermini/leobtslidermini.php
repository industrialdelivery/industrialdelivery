...
if( !class_exists("LeoBaseSourcemini") ){
	include_once(_PS_MODULE_DIR_.'leobtslidermini/libs/BaseSource.php');	
}
include_once(_PS_MODULE_DIR_.'leobtslidermini/libs/Slideshow.php');
include_once(_PS_MODULE_DIR_.'leobtslidermini/libs/Params.php');
if (!class_exists('PhpThumbFactory')) {
    require  _PS_MODULE_DIR_.'leobtslidermini/libs/phpthumb/ThumbLib.inc.php';
}

class leobtslidermini extends Module
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Leo bootstrap slider mini');
	...
	public function _prepareForm(){
		...
		$sources = LeoBaseSourcemini::getSourceList();
		foreach( $sources as $source ){
		
			$p = LeoBaseSourcemini::getSource($source)->getParams();
	...
	{
		/* Adds Module */
		if (parent::install() && $this->registerHook('DisplayContentBottom') && $this->registerHook('actionShopDataDuplication'))
		{
	...
	private function installSamples()
	{
		...
		for ($i = 1; $i <= 2; ++$i)
		{
			$slidemini = new leobtsliderminiControl();
			$slidemini->position = $i;
			$slidemini->active = 1;
			foreach ($languages as $language)
			{
				$slidemini->title[$language['id_lang']] = 'Design 2013 ';
				$slidemini->description[$language['id_lang']] = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.';
				$slidemini->legend[$language['id_lang']] = 'sample-'.$i;
				$slidemini->url[$language['id_lang']] = '#';
				$slidemini->image[$language['id_lang']] = 'sample-'.$i.'.jpg';
			}
			$slidemini->add();
		}
	}
	...
	{
		$slideminis = $this->getSlides();
		foreach ($slideminis as $slidemini)
		{
			$to_del = new leobtsliderminiControl($slidemini['id_slide']);
			$to_del->delete();
		}
	...
	private function _displayAddForm()
	{
		...
		if (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))
			$slidemini = new leobtsliderminiControl((int)Tools::getValue('id_slide'));
			
		//	echo '<pre>'.print_r( $slidemini, 1 ); die;
		...
		foreach ($languages as $language)
		{
			...
			$this->_html .= '<input type="file" name="image_'.$language['id_lang'].'" id="image_'.$language['id_lang'].'" size="30" value="'.(isset($slidemini->image[$language['id_lang']]) ? $slidemini->image[$language['id_lang']] : '').'"/>';
			/* Sets image as hidden in case it does not change */
			if ($slidemini && $slidemini->image[$language['id_lang']])
				$this->_html .= '<input type="hidden" name="image_old_'.$language['id_lang'].'" value="'.($slidemini->image[$language['id_lang']]).'" id="image_old_'.$language['id_lang'].'" />';
			/* Display image */
			if ($slidemini && $slidemini->image[$language['id_lang']])
				$this->_html .= '<img src="'.__PS_BASE_URI__.'modules/'.$this->name.'/images/'.$slidemini->image[$language['id_lang']].'" width="'.($this->getParams()->get('imgwidth',350)/2).'" height="'.($this->getParams()->get('imgheight',490)/2).'" alt=""/>';
			$this->_html .= '</div>';
		}
		...
		if ($slidemini && Tools::getValue('id_slide'))
			$this->_html .= '<input type="hidden" name="id_slide" value="'.$slidemini->id.'" id="id_slide" />';
		/* Sets position as hidden */
		$this->_html .= '<input type="hidden" name="position" value="'.(($slidemini != null) ? ($slidemini->position) : ($this->getNextPosition())).'" id="position" />';
		...
		foreach ($languages as $language)
		{
			$this->_html .= '
					<div id="title_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $id_lang_default ? 'block' : 'none').';float: left;">
						<input type="text" name="title_'.$language['id_lang'].'" id="title_'.$language['id_lang'].'" size="30" value="'.(isset($slidemini->title[$language['id_lang']]) ? $slidemini->title[$language['id_lang']] : '').'"/>
					</div>';
		}
		...
		foreach ($languages as $language)
		{
			$this->_html .= '
					<div id="url_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $id_lang_default ? 'block' : 'none').';float: left;">
						<input type="text" name="url_'.$language['id_lang'].'" id="url_'.$language['id_lang'].'" size="30" value="'.(isset($slidemini->url[$language['id_lang']]) ? $slidemini->url[$language['id_lang']] : '').'"/>
					</div>';
		}
		...
		foreach ($languages as $language)
		{
			$this->_html .= '
					<div id="legend_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $id_lang_default ? 'block' : 'none').';float: left;">
						<input type="text" name="legend_'.$language['id_lang'].'" id="legend_'.$language['id_lang'].'" size="30" value="'.(isset($slidemini->legend[$language['id_lang']]) ? $slidemini->legend[$language['id_lang']] : '').'"/>
					</div>';
		}
		...
		foreach ($languages as $language)
		{
			$this->_html .= '<div id="description_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $id_lang_default ? 'block' : 'none').';float: left;">
				<textarea name="description_'.$language['id_lang'].'" rows="10" cols="29">'.(isset($slidemini->description[$language['id_lang']]) ? $slidemini->description[$language['id_lang']] : '').'</textarea>
			</div>';
		}
		...
		<div class="margin-form">
			...
			<input type="radio" name="active_slide" id="active_on" '.(($slidemini && (isset($slidemini->active) && (int)$slidemini->active == 0)) ? '' : 'checked="checked" ').' value="1" />
			<label class="t" for="active_on">'.$this->l('Yes').'</label>
			<img src="../img/admin/disabled.gif" alt="No" title="No" style="margin-left: 10px;" />
			<input type="radio" name="active_slide" id="active_off" '.(($slidemini && (isset($slidemini->active) && (int)$slidemini->active == 0)) ? 'checked="checked" ' : '').' value="0" />
	...
	private function _postProcess()
	{
		...
		{
			$slidemini = new leobtsliderminiControl((int)Tools::getValue('id_slide'));
			if ($slidemini->active == 0)
				$slidemini->active = 1;
			else
				$slidemini->active = 0;
			$res = $slidemini->update();
		...
		elseif (Tools::isSubmit('submitSlide'))
		{
			...
			{
				$slidemini = new leobtsliderminiControl((int)Tools::getValue('id_slide'));
				if (!Validate::isLoadedObject($slidemini))
				{
				...
				$slidemini = new leobtsliderminiControl();
			/* Sets position */
			$slidemini->position = (int)Tools::getValue('position');
			/* Sets active */
			$slidemini->active = (int)Tools::getValue('active_slide');
			...
			foreach ($languages as $language)
			{
				if (Tools::getValue('title_'.$language['id_lang']) != '')
					$slidemini->title[$language['id_lang']] = pSQL(Tools::getValue('title_'.$language['id_lang']));
				if (Tools::getValue('url_'.$language['id_lang']) != '')
					$slidemini->url[$language['id_lang']] = pSQL(Tools::getValue('url_'.$language['id_lang']));
				if (Tools::getValue('legend_'.$language['id_lang']) != '')
					$slidemini->legend[$language['id_lang']] = pSQL(Tools::getValue('legend_'.$language['id_lang']));
				if (Tools::getValue('description_'.$language['id_lang']) != '')
					$slidemini->description[$language['id_lang']] = pSQL(Tools::getValue('description_'.$language['id_lang']));
					...
					in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
				{
					...
					if (isset($temp_name))
						@unlink($temp_name);
					$slidemini->image[$language['id_lang']] = pSQL(Tools::encrypt($_FILES['image_'.($language['id_lang'])]['name'].$salt).'.'.$type);
				}
				elseif (Tools::getValue('image_old_'.$language['id_lang']) != '')
					$slidemini->image[$language['id_lang']] = pSQL(Tools::getValue('image_old_'.$language['id_lang']));
			}
			...
			if (!$errors)
			{
				...
				elseif (!$slidemini->update())
					$errors .= $this->displayError($this->l('Slide could not be updated'));
			}
		} /* Deletes */
		elseif (Tools::isSubmit('delete_id_slide'))
		{
			$slidemini = new leobtsliderminiControl((int)Tools::getValue('delete_id_slide'));
			$res = $slidemini->delete();
			if (!$res)
	...
	private function _prepareHook()
	{
		...
		$slideminis = LeoBaseSourcemini::getSource( $source )
			->setModuleName('leobtslidermini', $path, $site_url )
