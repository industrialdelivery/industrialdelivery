...
$(document).ready(function() {
    $("{$call_owl_carousel|escape:'html':'UTF-8'}").owlCarousel({
            ...
            {if $owl_itemsCustom}itemsCustom : {$owl_itemsCustom|escape:'html':'UTF-8'},{/if}
            singleItem : false,         // true : show only 1 item
            itemsScaleUp : false,
            slideSpeed : {if $owl_slideSpeed}{$owl_slideSpeed|intval}{else}200{/if},  //  change speed when drag and drop a item
            paginationSpeed : {if $owl_paginationSpeed}{$owl_paginationSpeed|escape:'html':'UTF-8'}{else}800{/if}, // change speed when go next page
            rewindSpeed : {if $owl_rewindSpeed}{$owl_rewindSpeed|escape:'html':'UTF-8'}{else}1000{/if}, // change speed when carousel go back first page
