...
class WelcomeClass extends ObjectModel
{
	/** @var integer welcome id*/
	public $id;
	
	/** @var integer welcome id shop*/
	public $id_shop;
	...
	{
		$id = Db::getInstance()->getValue('SELECT `id_welcome` FROM `'._DB_PREFIX_.'welcome` WHERE `id_shop` ='.(int)$id_shop);
		if ($id)
			return new welcomeClass((int)$id);
		else
			return false;
	}
